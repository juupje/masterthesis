...
def plot(outdir:str, logger, model_name:str="Model", prediction_files:dict=None,config:dict=None, extra_info:dict=None,
    ...
    if(prediction_files is not None and len(prediction_files.keys())>0):
        ...
        for i,key in enumerate(prediction_files.keys()):
            ...
            y_true = np.concatenate((y_true_cr, y_true_sr), axis=0)


            #We want the SR vs. SB roc curves (not S vs. B!)
            sr_sb_label = np.concatenate((np.zeros(y_true_cr.shape[0]), np.ones(y_true_sr.shape[0])), axis=0)
            # and we only want this for true background events
            is_bg = y_true==0
            ...
            for func,name,brightness in zip((np.mean,np.median,np.max), ("mean", "median", "max"), (0.2,0.4,0.6)):
                ...
                y_pred = np.concatenate((y_pred_cr, y_pred_sr), axis=0)
                fpr, tpr, thres = roc_curve(sr_sb_label[is_bg], y_pred[is_bg], pos_label=1)
                ...
                auroc = roc_auc_score(sr_sb_label[is_bg], y_pred[is_bg])
                ...
                print(f"\tIn CR: (total: {y_pred_cr.shape[0]:d})")
                ...
                selected_in_cr = np.sum(is_selected_cr)
                print(f"\t\t{selected_in_cr:d} events selected in total")
                ...
                print(f"\tIn SR: (total: {y_pred_sr.shape[0]:d})")
                ...
                expected_background_in_sr = selected_in_cr*(y_pred_sr.shape[0]/y_pred_cr.shape[0])
                sigma = np.sqrt(selected_in_cr+expected_background_in_sr)
            ...
            for y1,y2 in zip(cr["y_preds"], sr["y_preds"]):
                y_pred = np.concatenate((y1,y2),axis=0)
                fpr, tpr, thres = roc_curve(sr_sb_label[is_bg], y_pred[is_bg], pos_label=1)
        ...
        if(logx):
            ...
            ax.set_xlim(0,1)
        ax.set_ylim(1,1e5)
        ax.set_xlabel(r"$\epsilon_{SR}$")
        ax.set_ylabel(r"$1/\epsilon_{SB}$")
        ax.grid()
        ax.legend()
        ax.set_title("SR vs. SB")
