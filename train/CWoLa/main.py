...
def get_SR_test_data():
    data_bg = h5py.File(os.path.join(config["DATA_DIR"], config["PREPROCESSED_TEST_DATA_BG_SR"]), mode='r')
    data_sn = h5py.File(os.path.join(config["DATA_DIR"], config["PREPROCESSED_TEST_DATA_SN_SR"]), mode='r')
    ...
    bg_slice = slice(*config.get("BG_SLICE_TEST",[None]))
    sn_slice = slice(*config.get("SN_SLICE_TEST",[None]))
    print(f"Using {bg_slice=}, {sn_slice=}")
    is_sr_background = disc.apply(data_bg)[bg_slice]
    is_sr_signal = disc.apply(data_sn)[sn_slice]
    ...
    bg = {jet: {x:np.array(data_bg[jet][x][bg_slice,:config["N"]])[idx_bg] for x in l} for jet in jets}
    sn = {jet: {x:np.array(data_sn[jet][x][sn_slice,:config["N"]])[idx_sn] for x in l} for jet in jets}
...
if __name__ == "__main__":
    ...
    if(config["PERFORM_TEST"]):
        ...
        do_SR_test = config.get("DO_SR_TEST",True)
