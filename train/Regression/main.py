...
from DataLoaders import Discriminator
...
def get_data():
    data = h5py.File(os.path.join(config["DATA_DIR"], config["PREPROCESSED_TRAIN_DATA"]))
    # For training
    ...
    train_slice = slice(0,config["BG_SLICE_TRAIN"])
    val_slice = slice(config["BG_SLICE_TRAIN"], config["BG_SLICE_TRAIN"]+config["BG_SLICE_VAL"])

    train_generator = RDL.RegressionDataLoader(data, batch_size, data_slice=train_slice, regression_feature=-1, particles=config["N"],
                                               log=True, shift_mean=True,
                                               N_data=n_train, features=config["FEATURES"], seed=config["SEED"], inputs=inputs, njets=2)
    val_generator = RDL.RegressionDataLoader(data, batch_size, data_slice=val_slice, regression_feature=-1, particles=config["N"],
                                            log=True, shift_mean=True,
...
def get_test_data():
    file = h5py.File(os.path.join(config["DATA_DIR"], config["PREPROCESSED_TEST_DATA"]), mode='r')
    ...
    print(f"Using {n_test} test events")
    data = {jet: {x:np.array(file[jet][x][:n_test,:config["N"]]) for x in l} for jet in jets}
    if(features != 'all' and features is not None):
        #select the required features
        for jet in jets:
            data[jet]["features"] = data[jet]["features"][:,:,features]
    target = np.log10(np.array(file["jet_features"][:n_test,-1]))
    file.close()
    return [data[jet][s] for jet in jets for s in l], target, np.arange(n_test)
...
if __name__ == "__main__":
    ...
    if(not args["test"]):
        if(args["resume"]):
            ...
            model = modelhandler.create_regression_model(config)
            initial_epoch = 0
    ...
    if(config["PERFORM_TEST"]):
        ...
        data, log_target, idx = get_test_data()
        target = np.power(10, log_target)
