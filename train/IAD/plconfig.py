...
SEED = 12365
# BATCH_SIZE = 500 set by model config file
VERBOSITY = 2
PERFORM_TEST = True
...
N_DATA_SIGNAL_TRAIN = 1000*factor # taken from data-sn
...
N_DATA_BACKGROUND_VAL = N_DATA_BACKGROUND_TRAIN # taken from data-bg
...
TAG = "LR_test"
COMMENT = 'Single cosine annealing. Different MAX_LR'
...
GRID = create_grid(lambda LR_MAX: f"LR={LR_MAX:.0e}", {"LR_MAX": [1e-4, 1e-3, 5e-4]})
#GRID = create_grid(lambda FEATURES: f"f={len(FEATURES):d}", {"FEATURES": [[0,1,2,3,6,7], [0,1,2,3,6], [0,1,2,6,7], [0,1,2,6]]})
