...
def get_data(test:bool=False):
    ...
    if(not test):
        train_generator = JDL.IADDataLoaderV2(sim_bg=data["simulated"], data_sn=data["data_signal"], data_bg=data["data_background"],batch_size=batch_size,
                                    N_simulated=n_sim_t, N_background=n_bg_t, N_signal=n_sn_t, features=config["FEATURES"], seed=data_seed,
        ...
        val_generator = JDL.IADDataLoaderV2(sim_bg=data["simulated"], data_sn=data["data_signal"],data_bg=data["data_background"], batch_size=batch_size,
                                    N_simulated=n_sim_v, N_background=n_bg_v, N_signal=n_sn_v, features=config["FEATURES"], seed=data_seed,
                                    noise_features=config.get("NOISE_FEATURES", None), noise_param=config.get("NOISE_PARAM", (0,1)), noise_type=config.get("NOISE_TYPE", "normal"),
                                    inputs=inputs, njets=2, discriminator=disc, oversampling=config["OVERSAMPLING"],
        ...
        test_generator = JDL.IADDataLoaderV2(sim_bg=data["simulated"], data_sn=data["data_signal"], data_bg=data["data_background"],batch_size=batch_size,
                                    ...
                                    noise_features=config.get("NOISE_FEATURES", None), noise_param=config.get("NOISE_PARAM", (0,1)), noise_type=config.get("NOISE_TYPE", "normal"),
                                    inputs=inputs, njets=2, discriminator=disc, oversampling=False,
...
def get_test_data():
    ...
    bg = {jet: {x:np.array(data_bg[jet][x][offset_bg:])[idx_bg] for x in l} for jet in jets}
    sn = {jet: {x:np.array(data_sn[jet][x][offset_sn:])[idx_sn] for x in l} for jet in jets}
